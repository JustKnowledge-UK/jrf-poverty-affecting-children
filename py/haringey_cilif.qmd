...
This script visualises Children in Low Income Families for small areas in Haringey.
...
Get geometry data required

```{python}
filepath = os.path.join(data_dir, 'ecm_trainline.pkl')
if not os.path.isfile(filepath):
    with engine.connect() as con:
        ...
        with open(filepath, 'wb') as file:
            pickle.dump(trainline, file)
else:
    with open(filepath, 'rb') as file:
        trainline = pickle.load(file)

filepath = os.path.join(data_dir, 'haringey_gpd.pkl')
if not os.path.isfile(filepath):
    with engine.connect() as con:
        ...
        with open(filepath, 'wb') as file:
            pickle.dump(haringey_gpd, file)
else:
    with open(filepath, 'rb') as file:
        haringey_gpd = pickle.load(file)

filepath = os.path.join(data_dir, 'msoa_gdf_haringey-cilif.pkl')
if not os.path.isfile(filepath):
    with engine.connect() as con:
...
filepath = os.path.join(data_dir, 'london_msoas.pkl')
if not os.path.isfile(filepath):
    with engine.connect() as con:
        ...
        with open(filepath, 'wb') as file:
                pickle.dump(london_msoas, file)
else:
    with open(filepath, 'rb') as file:
        london_msoas = pickle.load(file)

filepath = os.path.join(data_dir, 'haringey_msoas_haringey-cilif.pkl')
if not os.path.isfile(filepath):
    with engine.connect() as con:
        ...
        with open(filepath, 'wb') as file:
                pickle.dump(haringey_msoas, file)
else:
    with open(filepath, 'rb') as file:
        haringey_msoas = pickle.load(file)
```
...
Now get population estimates for denominators
...
fig.savefig(os.path.join('..','outputs','cilif_haringey.svg'), transparent=True)
```

jet version of above (for launch event)
...
plt.tight_layout()
fig.savefig(os.path.join('..','outputs','cilif_haringey_jet.png'), dpi=300, bbox_inches='tight')
fig.savefig(os.path.join('..','outputs','cilif_haringey_jet.svg'), transparent=True)


```

ice version wihtout ecm (launch event)
...
plt.tight_layout()
fig.savefig(os.path.join('..','outputs','cilif_haringey_wo_ecm.png'), dpi=300, bbox_inches='tight')

fig.savefig(os.path.join('..','outputs','cilif_haringey_wo_ecm.svg'), transparent=True)
```
...
# Explore East-West division

Split the msoas based on the ECM

```{python}
centroids = cilif_gdf.copy()
centroids["geometry"] = cilif_gdf.geometry.centroid

intersection = cilif_gdf.sjoin(ecm_clipped)
...
missing = cilif_gdf.copy()
missing = missing.loc[missing['msoa21cd'].isin(['E02000412', 'E02000432'])]
intersection = gpd.GeoDataFrame(pd.concat([intersection, missing], ignore_index=True))
intersection[['min_easting', 'min_northing', 'max_easting', 'max_northing']] = intersection.geometry.apply(lambda x: pd.Series(x.bounds))

intersection_msoas = intersection['msoa21cd'].unique().tolist()
...
east_msoas = [msoa for msoa in intersection_msoas if msoa not in west_msoas]

max_easting = intersection.loc[intersection['msoa21cd'].isin(west_msoas)]['min_easting'].max()
max_easting2 = intersection.loc[intersection['msoa21cd'].isin(west_msoas)]['max_easting'].max()

bounds = cilif_gdf.copy()

bounds[['min_easting', 'min_northing', 'max_easting', 'max_northing']] = cilif_gdf.geometry.apply(lambda x: pd.Series(x.bounds))

west_gdf = bounds.copy()

# I'm trying to get to a more sophisticated way of doing this without manual intervention. this was the best I could do with the time available.
# west_gdf = west_gdf.loc[west_gdf['min_easting'] <= max_easting]
west_gdf = west_gdf.loc[(west_gdf['min_easting'] <= max_easting) & (west_gdf['max_easting'] <= max_easting2)]

west_gdf = west_gdf.loc[~west_gdf['msoa21cd'].isin(east_msoas + ['E02000397', 'E02000403'])]

fig, ax = plt.subplots()
west_gdf.plot(ax=ax)
for idx, row in west_gdf.iterrows():
    x, y = row.geometry.centroid.coords[0]
    ax.text(x, y, row['msoa21cd'], fontsize=6, ha='center', va='center')

all_west_msoas = west_gdf['msoa21cd'].unique().tolist()

east_gdf = bounds.loc[~bounds['msoa21cd'].isin(all_west_msoas)]

fig, ax = plt.subplots()
east_gdf.plot(ax=ax)
for idx, row in east_gdf.iterrows():
    x, y = row.geometry.centroid.coords[0]
    ax.text(x, y, row['msoa21cd'], fontsize=6, ha='center', va='center')

```

Now summarise for east vs west

```{python}
# now we can quantify the cilif for east and west
average_cilif_west = west_gdf['percentage'].mean()
average_cilif_east = east_gdf['percentage'].mean()

print('west average: ' + str(round(average_cilif_west,0)))
print('east average: ' + str(round(average_cilif_east,0)))
```

Check we've got all the MSOAs. Should equal 36

```{python}
# check i've got all the msoas
all_east_msoas = east_gdf['msoa21cd'].unique().tolist()

# should = 36
len(all_east_msoas + all_west_msoas)
...
fig.savefig(os.path.join('..','outputs','cilif_haringey_decile_2023.png'), dpi=300)
```
...
fig.savefig(os.path.join('..','outputs','cilif_haringey_change_2014-23.png'), dpi=300)
```