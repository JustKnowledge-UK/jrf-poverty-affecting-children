...
# Plot UK premier league stadia on a map
...
Identify neighbours by creating a 1km buffer around each stadium
...
filepath = os.path.join(data_dir, 'msoa_lad_lookup21.pkl')
if not os.path.isfile(filepath):
    with psycopg2.connect(**db_params) as con:
        ...
        with open(filepath, 'wb') as file:
            pickle.dump(msoa_lad_lookup, file)

else:
    with open(filepath, 'rb') as file:
        msoa_lad_lookup = pickle.load(file)
...
ax.scatter(np_dots['relative_rank'], np_dots['stadium'], c=colours['highlight_colour']['colour'])
ax.scatter(la_avg_ranks['average_rank'], la_avg_ranks['Stadium'], marker="D",c=colours['lad_avg']['colour'])
...
ax.text(0, ax.get_ylim()[1]*1.15, s=f'{colours['highlight_colour']['name']} dot indicates Northumberland Park\nBlack dots indicate MSOAs within 1km of Tottenham Hotspur Stadium\n{colours['lad_avg']['name']} diamonds indicate the average rank of the borough the Stadium is in', size=8)
...
# plt.tight_layout()
fig.savefig(os.path.join('..','outputs','all_stadiums_1km.png'), dpi=300, bbox_inches='tight')
plt.show()
...
# ax.set_ylim(ymin - (ymax - ymin) * 0.02, ymax)

ax.margins(y=0.1)  # Adds 10% margin above and below
...
# # Expand the top by 10% (adjust as needed)
