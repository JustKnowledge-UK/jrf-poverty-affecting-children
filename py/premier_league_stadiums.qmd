...
```{python}
relative = pd.read_csv(os.path.join('..','data','relative_england_wales_msoa_23-24_codes.csv'), skiprows=9, skipfooter=15)

relative = relative.iloc[:,[1,3]]
relative.columns = ['msoa_code','count']
...
ax.text(0, ax.get_ylim()[1]*1.1, s='Red dot indicates Northumberland Park\nBlack dots indicate MSOAs within 1km of Tottenham Hotspur Stadium', size=8)
...
plt.suptitle(
    'MSOAs within 1km of a Stadium', fontsize=14, 
    x = 0.7, y=1.03, 
    ha='center')  # y controls vertical position
fig.savefig(os.path.join('..','outputs','stadiums_1km.png'), dpi=800)
plt.show()
