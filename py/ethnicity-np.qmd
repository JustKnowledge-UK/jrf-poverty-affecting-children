---
title: "Ethnicity breakdown of Northumberland Park"
format: 
    html:
        code-fold: true
        css: ../styles.css
...
This script acquires and processes data on ethnicity and age of arrival for Northumberland Park
...
geography = 'geography=E02000398'

# Build the query
query = base + date + "&" + geography + "&" + ethn + "&" + measures

filepath = os.path.join(data_dir, 'np_ethnicity.csv')
...
# Save the agg cats
data_agg = data.loc[~data['c2021_eth_20_code'].str.contains('_')] 
data_agg = data_agg.loc[data_agg['ethnicity_aggregated']!='Total']
data_agg['percent'] = data_agg['obs_value'].div(data_agg['obs_value'].sum()) * 100
...
Plot ethnicity breakdown for NP
...
Here we just want to find out what percentage of residents in Northumberland Park were born/not born in the UK.

```{python}
base = 'https://www.nomisweb.co.uk/api/v01/dataset/NM_2038_1.data.csv?'

date = 'date=latest'
geography = 'geography=E02000398'
age = 'c2021_agearruk_18=0,1,1001,2...17'
measures = 'measures=20100'


# Build the query
query = base + date + "&" + geography + "&" + age + "&" + measures

filepath = os.path.join(data_dir, 'np_age-of-arrival.csv')
if not os.path.isfile(filepath):
    response = requests.get(query)
    with open(filepath, 'wb') as file:
        file.write(response.content)
...
aof_data = pd.read_csv(filepath)

aof_data = janitor.clean_names(aof_data) 


cols = ['c2021_agearruk_18_code','c2021_agearruk_18_name', 'obs_value']

aof_data = aof_data[cols]
aof_data = aof_data.loc[aof_data['c2021_agearruk_18_code'].isin(['_1','1001'])]
aof_data['percentage'] = aof_data['obs_value'].div(aof_data['obs_value'].sum()) * 100
