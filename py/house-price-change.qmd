...
Here we plot house price deciles and change between 2000-2023. Data are proprietary which is why they are loaded from csv rather than a public source.
...
filepath = os.path.join(data_dir, 'np_boundaries.pkl')
if not os.path.isfile(filepath):
    with engine.connect() as con:
        ...
        with open(filepath, 'wb') as file:
            pickle.dump(np_gdf, file)
else:
    with open(filepath, 'rb') as file:
        np_gdf = pickle.load(file)
```

# Plot decile change 2000-2023
...
# Plot house price deciles in 2023
...
# add outline of northumberland park
np_gdf.boundary.plot(ax=ax, color=jk_colours[5], linewidth=1)
...
Get the percentage of Output areas in Northumberland Park in each decile

```{python}
filepath = os.path.join(data_dir, 'np_oa_lookup.pkl')
if not os.path.isfile(filepath):
    with engine.connect() as con:
        ...
        with open(filepath, 'wb') as file:
            pickle.dump(np_oas, file)

else:
    with open(filepath, 'rb') as file:
        np_oas = pickle.load(file)
...
np_summary = np_summary.to_frame('count').reset_index(drop=False)  # optional: turn it back into a DataFrame
np_summary['percentage'] = np_summary['count'] / np_summary['count'].sum() * 100
np_summary = np_summary.rename(columns={'year_2023': 'decile'})
